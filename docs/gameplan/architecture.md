...
- **Chart-start policy:** keep tick 0 anchored to the GP score's own timeline. If a chart engine requires nonnegative event ticks, add optional pre-roll/count-in space ahead of musical content rather than translating individual event classes independently. Notes, tempo events, time signatures, and section markers must all receive the same timeline transform.
