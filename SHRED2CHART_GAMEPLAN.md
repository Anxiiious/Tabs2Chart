...
- 2026-07-21 — **Mapper reworked: chord voicing removed, contour tracker rebuilt as a directional wraparound cursor** (Claude Code session, branch `claude/mapper-contour-wraparound-lceuej`). Three cumulative changes to `shred2chart/mapper.py`:
  - **Chord interval-spread voicing removed.** The absolute-interval chord logic (power chords → adjacent lanes, wider voicings → wider spread, max-3-lane capping) was the actual source of mapping bugs; single-note contour mapping was fine. Every note, chord member or not, now goes through the same per-note lane assignment path.
  - **Distinct-lane guarantee for same-tick notes.** Same-tick notes are grouped and lane collisions resolved by nudging to the nearest still-free lane (0-4), processed lowest-pitch-first so the anchor note claims its natural spot — a naive set-union merge could silently dedupe two genuinely different simultaneous notes onto one lane.
  - **Contour tracker rebuilt: windowed min/max → directional wraparound cursor.** The old tracker capped out — a long rising run pinned at orange (lane 4) and flatlined. New mechanism: an unbounded running cursor; each new distinct pitch moves it by a signed step (magnitude from semitone interval via `_interval_to_step`, direction from interval sign); the visible lane is `cursor % 5`, giving the wraparound "Ladder"/staircase pattern. Cursor resets to 0 (green) on phrase boundaries (section marker tick, or rest ≥ 1 bar) — same reset triggers as before.
  - **Validation (research):** "Ladder" is a named Clone Hero community convention (official CH Wiki dictionary: https://wiki.clonehero.net/books/general-info/page/dictionary). The anchor+motion+wraparound mechanism independently matches the Berkeley "Tensor Hero" paper's reverse-engineered charting model (450 mined GH/CH charts, motion range [-4,4], same span as ours): https://www.ischool.berkeley.edu/sites/default/files/sproject_attachments/tensorhero_capstone.pdf. The "preserve melodic motion over strict pitch consistency" principle is confirmed verbatim by Rock Band Network's authoring docs (http://docs.c3universe.com/rbndocs/index.php?title=Guitar_and_Bass_Authoring), which also confirm gem-wrapping for runs exceeding 5 lanes.
  - **Validation (code):** verified a synthetic 12-note rising run staircases 0→4, wraps back to 0, and keeps climbing (previously it would flatline at 4). Repeated identical pitches stay on one lane (fixture smoke test via `shred2chart convert tests/fixtures/sample.gp`). Chord-voicing tests rewritten to assert the new behavior (all notes kept, distinct lanes, no 3-lane cap) — the old tests asserted intentionally-removed behavior, per this session's handoff.
  - **KNOWN UNRESOLVED (flagged, deliberately not fixed):** the placeholder branch in `_assign_group_lanes` anchors extra chord-note lanes off the first assigned lane in the group — which can be OPEN_NOTE (7) when the chug rule fires, causing the fretted chord notes to collide on lane 7 and dedupe. Captured as a `strict=True` xfail test (`test_open_chug_chord_keeps_all_notes`). Chord logic is explicitly deprioritized; the right behavior hasn't been decided — review before trusting against chord-bearing files.
  - **Open question, NOT implemented:** step-size-by-detected-run — real charters may force a uniform 1-lane step for monotonic runs (3+ notes, same direction) regardless of semitone gap, reserving proportional `_interval_to_step` sizing for leaps. Would need a run-detection pre-pass; waiting on concrete evidence (a real chart example). The `_interval_to_step` thresholds are our own heuristic, not confirmed against real chart data.
  - **59 tests pass, 1 xfail** (`pytest`). Still pending: `convert` against real `.gp` files in `test_data/` (none present in this environment — ask the user) to confirm staircasing on real solos/runs.
...
- 2026-07-21 — **Chord interval-spread voicing removed from the mapper.** It was the source of the mapping bugs, and real-chart research (RBN authoring docs, Tensor Hero) supports per-note contour motion as the primary mechanism. Chord lane placement beyond the anchor note is now an explicit placeholder pending a real decision — see Current State. Note: the 2026-07-20 open-note-rule decision's rationale referenced "the chord voicing path"; that path no longer exists, but the decision itself (fret 0 on the single lowest-tuned string only) stands unchanged.
- 2026-07-21 — **Windowed min/max contour replaced by directional wraparound cursor** (`cursor % 5`, signed steps from semitone intervals, reset to 0 on phrase boundaries). Rationale and research citations in Current State. Cursor always resets to 0 (green) regardless of whether the next phrase ascends or descends — considered and kept, based on real-guitar fretting-hand ergonomics (anchor stays low; higher notes are a temporary reach).
...
- New (2026-07-21): step-size-by-detected-run — should monotonic runs (3+ notes, same direction) force a uniform 1-lane step regardless of semitone gap, with proportional `_interval_to_step` sizing reserved for leaps between runs? Partially research-supported, not confirmed; needs a run-detection pre-pass that doesn't exist yet. Do not implement without concrete evidence (a real chart example would settle it).
- New (2026-07-21): chord-note lane placement beyond the anchor note in `_assign_group_lanes` is a placeholder (anchors off the first assigned lane in the group, which can be an OPEN_NOTE lane). Known-broken for open-chug chords (see xfail test); the right behavior is undecided.
