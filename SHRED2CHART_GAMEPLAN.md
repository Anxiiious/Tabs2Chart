...
- [~] **M4 — Real mapping:** v1 (threshold-based static-anchor contour) shipped, playtested, and found broken (wide lead runs jammed onto 2 lanes) — v2 (proportional lane-distance step) fixes it, verified against the real breaking case. See Current State 2026-07-19 (both entries). Awaiting the human playtest of v2 (a `--track` single-track conversion of `Still Searching` is ready).
...
- [~] **M5b — CLI polish:** `--offset-ms` flag done; `--lead-in-bars` (default 2) done. Still open: batch mode, song.ini metadata from GP file header.
...
- 2026-07-19 — **M4 contour anchor: static (threshold-based), not always-updating.** User explicitly chose a design where the anchor only moves on a >4-semitone leap, rejecting the simpler "update the anchor after every note" alternative. Reason given: the anchor should model a guitarist's actual hand position, which doesn't shift for every note played within reach — and the static design is what gives pattern stability (identical riffs map identically), which an always-updating anchor can't guarantee. If lane choices ever look wrong in a future playtest, don't revert to always-update as the fix — that reintroduces the exact non-determinism this design was chosen to avoid; ~~tune `HAND_POSITION_SEMITONES` instead~~ **correction, same day:** this last sentence was itself wrong — the real M4 v1 bug (see the follow-up Current State entry) wasn't `HAND_POSITION_SEMITONES`, it was the flat `±1` step size applied once a leap re-centered the anchor. `HAND_POSITION_SEMITONES` was fine all along and is unchanged in v2; the actual fix was making the lane step **proportional** to leap distance (`SEMITONES_PER_LANE`). The static-anchor decision itself (the point of this entry) still stands.
- 2026-07-19 — **Kept the existing chord base-lane modulus (`% (5 - (width-1))`) over a hardcoded `% 3` that appeared in a pasted Gemini code sample.** The hardcoded version doesn't adapt to chord width and can push a 3-note chord's lanes out of the valid 0-4 range. General note for future sessions: when the user shares AI-generated code suggestions (Gemini, ChatGPT, etc.), treat them as a second reference for the *idea*, not literal code to paste in — verify function signatures, existing logic, and edge-case math against the real files before adopting, the same as any other unverified source. See this session's mapper.py rewrite for a worked example of doing this correctly (also caught: the pasted sample's own test suite asserted a 12-semitone chord gap as "close" in one place and used a 14-semitone example as "disjoint" elsewhere without ever testing the actual `>12` boundary its code implemented).
...
- New (2026-07-19): the repeat/D.S. al Coda timing fix has been verified mathematically (section tick → wall-clock seconds matches Sheet Happens' embedded timestamps to within about a second, for all 13 sections of `Still Searching`) but **not yet by an actual in-game playtest** with the lead-in applied. Next session: get the user's playtest result on the regenerated `songs/Senses Fail - Still Searching/` folder and log it here either way.
...
- New (2026-07-19, updated same day): `mapper.HAND_POSITION_SEMITONES` (4), `mapper.SEMITONES_PER_LANE` (3, added in M4 v2), and `mapper.DISJOINT_CHORD_SEMITONES` (12) are all starting-point constants, not tuned against real in-game playtest feedback yet — M4 v2's mapping is verified programmatically against the real sequence that broke v1 (see Current State), but hasn't had an actual in-game playtest. If lanes still look off after trying it, `SEMITONES_PER_LANE` is the most likely knob to need adjusting first (it directly controls how much a leap spreads across lanes) — `HAND_POSITION_SEMITONES` turned out NOT to be the fix for the v1 "stuck high" bug despite an earlier Decision Log entry suggesting it, so don't reach for that one first without re-deriving why.
