...
- 2026-07-17 — **Started M1** (per-note IR). Researched the real `score.gpif` note schema directly from `Brag.gpif` and `Boundaries.gpif` (both already on disk from M0 work): top-level `<Bars>`/`<Voices>`/`<Beats>`/`<Notes>`/`<Rhythms>` collections cross-referenced by id (see `shred2chart/ir_gpif.py`'s module docstring for the full shape). Built two parallel extractors matching §4's per-note IR fields (tick, duration_ticks, pitch, string, fret, chord_id, technique flags):
  ...
  - **Not done, left for next session:** the milestone's mandated verification step (open the tab in actual Guitar Pro, eyeball note count/positions against the IR dump) needs a human with the GUI. Also not attempted: hammer-on vs. pull-off direction (both sources give a flat `hopo` flag; direction is normally inferred from pitch-comparing consecutive notes, deferred to Stage 4), and `slide_in`/`slide_out` direction for the GPIF path specifically (PyGuitarPro gives a real enum for this; GPIF's `Slide`/`Flags` int is exposed raw since its bit meaning is still unconfirmed — see Open Questions).
...
- New (2026-07-17): what do the `<Slide><Flags>` integer values in GPIF actually mean, bit for bit? Only one real value seen so far (`2`). PyGuitarPro's `SlideType` enum (intoFromAbove/intoFromBelow/shiftSlideTo/legatoSlideTo/outDownwards/outUpwards) is presumably what GP7 encodes as bit flags here, but the mapping is a guess until more real examples turn up. Until then `ir_gpif.py` exposes the raw `slide_flags` int rather than committing to `slide_in`/`slide_out` booleans like `ir_gp.py` does.
