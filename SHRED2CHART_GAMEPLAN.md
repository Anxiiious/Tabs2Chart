...
- ~~What do the `<Slide><Flags>` integer values in GPIF actually mean, bit for bit?~~ **Resolved 2026-07-17** — cross-checked against editor-on-fire's GP importer, which documents the GP5+ bitmask (1/2/4/8/16/32). See Current State. Still only one real GP7 example (`Flags=2`) confirms GP7 reuses it unchanged; a second real slide example would firm this up further.
