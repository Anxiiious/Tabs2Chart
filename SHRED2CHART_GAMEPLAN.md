...
- [~] **M4 — Real mapping:** threshold-based static-anchor contour for single notes (replaces pitch-mod-5), plus a disjoint-chord gap refinement, built and unit-tested — see Current State 2026-07-19. Awaiting the human playtest (a `--track` single-track conversion of `Still Searching` is ready; note-mapping sanity-checked programmatically against real pitch sequences but not yet played in-game).
...
- New (2026-07-19): `mapper.HAND_POSITION_SEMITONES` (4) and `mapper.DISJOINT_CHORD_SEMITONES` (12) are both starting-point constants, not tuned against real playtest feedback yet — M4's contour mapping is built and programmatically verified against real note sequences (see Current State), but hasn't had an actual in-game playtest. If lanes still look off after trying it, these two constants are the first knobs to adjust before considering a bigger algorithm change.
